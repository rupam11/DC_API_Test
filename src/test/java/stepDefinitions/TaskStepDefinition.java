...
public class TaskStepDefinition extends Utils {
	...
	public void task_Payload_with_Param(String userRole, String payloadReq, String param) throws IOException {
		if (payloadReq.equalsIgnoreCase("Add")) {
			...
			reqTask = data.updateTask(respTask);
			reqSpec = null;
			reqSpec = given().spec(requestSpecification(userRole)).body(reqTask).queryParam(param,
					respTask.getTaskId());
	...
	public void verify_Task_fields_gets_updated() {
		...
		JSONAssert.assertEquals(new JSONObject(reqTask).toString(), new JSONObject(respTask).toString(),
				new CustomComparator(JSONCompareMode.LENIENT, new Customization("taskUpdatedAt", (o1, o2) -> true),
						new Customization("taskUpdatedBy", (o1, o2) -> true)));
	}

	@Given("{string} User {string} Task Payload  with no Task Body and Param = {string}")
	public void user_Task_Payload_with_no_Task_Body_and_Param(String userRole, String payloadReq, String param)
			throws IOException {
		reqSpec = null;
	...
	public void user_invoke_getTaskBySearchCriteria_with_Parameter(String userRole, String param1, String param2,
		...
		reqSpec = given().spec(requestSpecification(userRole)).queryParams(param1, respTask.getTaskId())
				.queryParam(param2, respTask.getTaskDescription()).queryParam(param3, respTask.getTaskApplicable())
	...
	public void user_Task_Payload_with_invalid_Param_and_value(String userRole, String payloadReq, String param, int i)
			throws IOException {
		reqTask = data.updateTask(respTask);
		reqSpec = null;
