...
public class CategoryStepDefinition extends Utils {
	...
	/*
	 * @Then("verify response will return Category instance") public void
	 * verify_response_will_return_Category_instance() { respCategory =
	 * response.getBody().as(Category.class);
	 * 
	 * cmnSteDef.user_calls_API_with_http_Request("getCategoryCount", "Get"); int
	 * expCatId = cmnSteDef.getcount_from_Response_and_verify_it_is_0(); int
	 * actCatId = respCategory.getCategoryId();
	...
	public void verify_Category_with_NoSearchCriteria() {
		...
		assertArrayEquals("All Categories not searched in case of no Search Criteria", expCategoryList,
				actCategoryList);
	}
	...
	public void response_should_be_list_of_Category_and_satisfy_the_search(String srchCriteria) {
		...
		if (srchCriteria.equalsIgnoreCase("categoryName")) {
			for (Category cat : respAllCategory)
				...
				assertTrue("Wrong list of category searched back",
						respCategory.getCategoryName().equalsIgnoreCase(cat.getCategoryName()));
		} else if (srchCriteria.equalsIgnoreCase("categoryStatus")) {
			for (Category cat : respAllCategory)
				assertTrue("Wrong list of category searched back",
						respCategory.getCategoryStatus().equalsIgnoreCase(cat.getCategoryStatus()));
		}
	...
	public void verify_Response_should_not_contain_Value_passed_in_Invalid_Parameter() {
		...
		for (Category cat : respAllCategory)
			...
			assertFalse("Response not satisfying the getCategoryNotinList Criteria",
					cat.getCategoryParentId().equalsIgnoreCase(srchCriteria));
	...
	public void verify_Category_fields_gets_updated() {
		...
		// assertEquals(updatedCategory.getCategoryUpdatedBy(),
		// respCategory.getCategoryUpdatedBy());
		// assertEquals(updatedCategory.getCategoryUpdatedAt(),
		// respCategory.getCategoryUpdatedAt());
	...
	public void user_invoke_CategoryAPI_getCategoryNotinList_with_Parameter(String userRole, String params)
			throws IOException {
		/*
		 ...
		 * 
		 * String [] arrParam=dataParam.split(","); System.out.println(arrParam);
		 * for(int i=0; i<arrParam.length; i++) {
		 * 
		 * System.out.println(arrParam[i]);
		 * //user_invoke_category_with_Parameter(userRole,"getCategoryNotinList",
		 * arrParam[i]);
	...
	public void user_invoke_api_with_invalid_Category_Parameter(String userRole, String apinum, String param,
