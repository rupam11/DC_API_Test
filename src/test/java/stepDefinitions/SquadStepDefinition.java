...
public class SquadStepDefinition extends Utils {
	...
	public void verify_Squad_fields_gets_updated() {
		...
		JSONAssert.assertEquals(new JSONObject(reqSquad).toString(), new JSONObject(respSquad).toString(),
				new CustomComparator(JSONCompareMode.LENIENT, new Customization("squadUpdatedAt", (o1, o2) -> true),
	...
	public void user_invoke_getTaskBySearchCriteria_with_Parameter(String userRole, String param1, String param2,
		...
		reqSpec = given().spec(requestSpecification(userRole)).queryParams(param1, respSquad.getSquadId())
				.queryParam(param2, respSquad.getSquadName()).queryParam(param3, respSquad.getSquadStatus());
		;
	...
	public void user_Squad_Payload(String userRole, String payloadReq, String param) throws IOException {
		if (payloadReq.equalsIgnoreCase("Add")) {
			...
			reqSquad = data.updateSquad(respSquad);
			reqSpec = null;
			reqSpec = given().spec(requestSpecification(userRole)).body(reqSquad).queryParam(param,
					respSquad.getSquadId());
	...
	public void user_Squad_Payload_with_no_Squad_Body_and_Param(String userRole, String payloadReq, String param)
			throws IOException {
		reqSpec = null;
		reqSpec = given().spec(requestSpecification(userRole));
