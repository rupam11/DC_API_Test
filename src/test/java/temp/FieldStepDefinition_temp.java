...
public class FieldStepDefinition_temp extends Utils {
	...
	public FieldStepDefinition_temp() throws IOException {
	...
	public void field_added_exist_in_returned_FieldList() {
		...
		System.out.println("Chek it later");
	...
	//@Given("Get query Param ready as queryParam.FieldId= {string}, queryParam.RuleId= {string}")
	@Given("{string} Get query Field-> {string} ->Parameters ready for {string}")
	//@Given("{string} Get query Field->Parameters ready for {string}")
	public void user_FieldId_RuleId(String fieldApiNm,String apiNm, String paramList) throws IOException {
		queryParams=new HashMap<String,Object>();
		//cmnStepDef.user_calls_API_with_http_Request(fieldApiNm, "Get");
	//	respAllFields = response.getBody().as(Field[].class);
		int fieldId=respField.getFieldId();
		
		
		//cmnStepDef.user_calls_API_with_http_Request(ruleApiNm, "Get");
		//respAllRules = response.getBody().as(Rule[].class);
		String ruleParentId=rule.getRuleParentId();
		
		
		queryParams.put("fieldId", fieldId);
		queryParams.put("ruleParentId", ruleParentId);
		
//		reqSpec=null;
//		reqSpec = given().spec(requestSpecification(userRole)).queryParams(queryParam);
	}
	
	/*@When("{string} calls updateFieldByRuleId API with Put http Request")
	public void user_calls_updateFieldByRuleId_API_with_Put_http_Request(String userRole) throws IOException {
		reqSpec=null;
		reqSpec = given().spec(requestSpecification(userRole)).queryParams(queryParam);
	...
	public void verify_fieldId_value(String param) {
		...
		/*if(
				(respField.getFieldId()==respField.getFieldDisplaySequence()) && 
				(respField.getFieldId()==Integer.parseInt(respField.getFieldParentId())) &&
				(respField.getFieldId()==get_field_count())
				){
			System.out.println("all matched");	
		}
		else
			System.out.println("comparison issue");*/
		
		/* assertTrue("FieldId/ FieldDisplaySequene/ fieldParentId mismatched!!!",
    			 (respField.getFieldId()==respField.getFieldDisplaySequence()) &&
    			 (respField.getFieldId()==Integer.parseInt(respField.getFieldParentId()) &&
    			 (respField.getFieldId()==get_field_count())));*/
	}
