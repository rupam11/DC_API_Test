...
public class CommonStepDefinition extends Utils {
	...
	/*
	@Given("{string} User gets query Parameters ready {string}")
	public void user_gets_query_Parameters_ready(String userRole, String queryParamList) {
		String[] queryParamArr = queryParamList.split(",");
		
		for(int i=0;i<queryParamArr.length;i++)
			queryParam.put(queryParamArr[i], value)
	    
	}*/

	//@When("User calls {string} API with {string} http Request {string}")
